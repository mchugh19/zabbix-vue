...
browser.runtime.onInstalled.addListener( async (details) => {
  ...
  //browser.alarms.onAlarm.addListener(getAllTriggers());
  await initalize();
});
*/
...
async function setActiveTriggersTable() {
  ...
  triggerResults = triggerResults["triggerResults"];
  let settings = {};
  browser.storage.local.get("ZabbixServers").then((results) => {
    settings = results;
  })
  let topSeverity = -1;
  let popupHeaders = [
  ...
  switch (request.method) {
    ...
    case "submitPagination":
      ...
      if (updated) {
        ...
        await browser.storage.local.set("ZabbixServers", settings);
      }
      await setActiveTriggersTable();
      break;
  }
